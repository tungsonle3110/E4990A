...
class E4990A_cv_messung:
    def __init__(self):
        ...
        self.dc_bias_voltage_level_start_value = 0
        self.dc_bias_voltage_level_stop_value = 0
        self.delay_value = 0.5
        self.sampling_dc_bias_voltage_level_value = None
        self.number_of_point_udc_value = 1
        self.frequency_value = 1000
        ...
        self.dc_bias_state_frame = Frame(self.input_frame)
        self.dc_bias_state_frame.pack(fill=X, expand=0)
        self.dc_bias_state_label = Label(self.dc_bias_state_frame, text="DC Bias State", width=16, anchor=W)
        self.dc_bias_state_label.pack(side=LEFT, fill=X, expand=1, pady=2)
        self.dc_bias_state_off = Radiobutton(self.dc_bias_state_frame, text="OFF", variable=self.dc_bias_state_value,
                                             value=0, indicator=0)
        self.dc_bias_state_off.pack(side=LEFT, fill=X, expand=1, pady=2)
        self.dc_bias_state_on = Radiobutton(self.dc_bias_state_frame, text="ON", variable=self.dc_bias_state_value,
                                            value=1, indicator=0)
        self.dc_bias_state_on.pack(side=LEFT, fill=X, expand=1, pady=2)

        self.dc_bias_start_frame = Frame(self.input_frame)
        self.dc_bias_start_frame.pack(fill=X, expand=0)
        self.dc_bias_voltage_level_start_label = Label(self.dc_bias_start_frame,
                                                       text="DC Bias Start(-40-40V)", width=20, anchor=W)
        self.dc_bias_voltage_level_start_label.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.dc_bias_voltage_level_start_entry = Entry(self.dc_bias_start_frame, validate="key", width=20)
        self.dc_bias_voltage_level_start_entry.config(
            validatecommand=(self.dc_bias_voltage_level_start_entry.register(validate_n), '%P'))
        self.dc_bias_voltage_level_start_entry.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.dc_bias_voltage_level_start_entry.insert(END, str(self.dc_bias_voltage_level_start_value))
        self.dc_bias_voltage_level_start_entry.bind('<Return>', lambda event: None)

        self.dc_bias_stop_frame = Frame(self.input_frame)
        self.dc_bias_stop_frame.pack(fill=X, expand=0)
        self.dc_bias_voltage_level_stop_label = Label(self.dc_bias_stop_frame,
                                                      text="DC Bias Stop(-40-40V)", width=20, anchor=W)
        self.dc_bias_voltage_level_stop_label.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.dc_bias_voltage_level_stop_entry = Entry(self.dc_bias_stop_frame, validate="key", width=20)
        self.dc_bias_voltage_level_stop_entry.config(
            validatecommand=(self.dc_bias_voltage_level_stop_entry.register(validate_n), '%P'))
        self.dc_bias_voltage_level_stop_entry.pack(side=LEFT, fill=X, expand=2, padx=2, pady=2)
        self.dc_bias_voltage_level_stop_entry.insert(END, str(self.dc_bias_voltage_level_stop_value))
        self.dc_bias_voltage_level_stop_entry.bind('<Return>', lambda event: None)

        self.delay_frame = Frame(self.input_frame)
        self.delay_frame.pack(fill=X, expand=0)
        self.delay_label = Label(self.delay_frame, text="Delay[s]", width=20, anchor=W)
        self.delay_label.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.delay_entry = Entry(self.delay_frame, validate="key", width=20)
        self.delay_entry.config(
            validatecommand=(self.delay_entry.register(validate_p), '%P'))
        self.delay_entry.pack(side=LEFT, fill=X, expand=2, padx=2, pady=2)
        self.delay_entry.insert(END, str(self.delay_value))
        self.delay_entry.bind('<Return>', lambda event: None)
        ...
        self.sampling_mode_value.set(1)

        self.sampling_dc_bias_voltage_level_label = Label(self.sampling_frame, text="\u0394Udc", anchor=W)
        self.sampling_dc_bias_voltage_level_label.pack(side=LEFT, fill=X, expand=0, padx=2, pady=2)
        self.sampling_dc_bias_voltage_level_entry = Entry(self.sampling_frame, validate="key", width=8)
        self.sampling_dc_bias_voltage_level_entry.config(
            validatecommand=(self.sampling_dc_bias_voltage_level_entry.register(validate_p), '%P'))
        self.sampling_dc_bias_voltage_level_entry.pack(side=LEFT, fill=X, expand=0, padx=2, pady=2)
        self.sampling_dc_bias_voltage_level_entry.config(state=DISABLED)
        self.sampling_dc_bias_voltage_level_entry.bind('<Return>', lambda event: None)

        self.number_of_point_udc_label = Label(self.sampling_frame, text="N(0-100000)", anchor=W)
        self.number_of_point_udc_label.pack(side=LEFT, fill=X, expand=0, padx=2, pady=2)
        self.number_of_point_udc_entry = Entry(self.sampling_frame, validate="key", width=1)
        self.number_of_point_udc_entry.config(
            validatecommand=(self.number_of_point_udc_entry.register(validate_p), '%P'))
        self.number_of_point_udc_entry.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.number_of_point_udc_entry.insert(END, str(self.number_of_point_udc_value))
        self.number_of_point_udc_entry.bind('<Return>', lambda event: None)

        self.frequency_frame = Frame(self.input_frame)
        self.frequency_frame.pack(fill=X, expand=0)
        self.frequency_label = Label(self.frequency_frame, text="Frequency [Hz]", width=20, anchor=W)
        self.frequency_label.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.frequency_entry = Entry(self.frequency_frame, validate="key", width=20)
        self.frequency_entry.config(validatecommand=(self.frequency_entry.register(validate_p), '%P'))
        self.frequency_entry.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.frequency_entry.insert(END, str(self.frequency_value))
        self.frequency_entry.bind('<Return>', lambda event: None)

        self.number_of_point_frame = Frame(self.input_frame)
        self.number_of_point_frame.pack(fill=X, expand=0)
        self.number_of_point_label = Label(self.number_of_point_frame, text="Points (2-1601)", width=20, anchor=W)
        self.number_of_point_label.pack(side=LEFT, fill=X, expand=1, padx=2, pady=2)
        self.number_of_point_entry = Entry(self.number_of_point_frame, validate="key", width=20)
        ...
        self.run_button = Button(self.run_frame, text="Run", command=self.run)
    ...
    def sampling_mode(self, sampling_mode_value):
        if sampling_mode_value == "0":
            self.sampling_dc_bias_voltage_level_entry.config(state=NORMAL)
            self.number_of_point_udc_entry.config(state=DISABLED)
        else:
            self.sampling_dc_bias_voltage_level_entry.config(state=DISABLED)
            self.number_of_point_udc_entry.config(state=NORMAL)
    ...
    def set_input(self):
        try:
            self.dc_bias_voltage_level_start_value = float(self.dc_bias_voltage_level_start_entry.get())
            if self.dc_bias_voltage_level_start_value < -40:
                self.dc_bias_voltage_level_start_value = -40
            elif self.dc_bias_voltage_level_start_value > 40:
                self.dc_bias_voltage_level_start_value = 40
            else:
                self.dc_bias_voltage_level_start_value = round(self.dc_bias_voltage_level_start_value, 3)
        except ValueError:
            pass

        try:
            self.dc_bias_voltage_level_stop_value = float(self.dc_bias_voltage_level_stop_entry.get())
            if self.dc_bias_voltage_level_stop_value < -40:
                self.dc_bias_voltage_level_stop_value = -40
            elif self.dc_bias_voltage_level_stop_value > 40:
                self.dc_bias_voltage_level_stop_value = 40
            else:
                self.dc_bias_voltage_level_stop_value = round(self.dc_bias_voltage_level_stop_value, 3)
        except ValueError:
            pass

        try:
            self.delay_value = float(self.delay_entry.get())
        except ValueError:
            pass

        if self.sampling_dc_bias_voltage_level_entry.config("state")[-1] == NORMAL:
            min_sampling_frequency_value = (self.dc_bias_voltage_level_stop_value
                                            - self.dc_bias_voltage_level_start_value) / 100000
            max_sampling_frequency_value = 40
            try:
                self.number_of_point_udc_value = round(float(self.number_of_point_udc_entry.get()))
                self.number_of_point_udc_entry.config(state=NORMAL)
                self.number_of_point_udc_entry.delete(0, END)
                self.number_of_point_udc_entry.insert(END, str(self.number_of_point_udc_value))
                self.number_of_point_udc_entry.config(state=DISABLED)
            except ValueError:
                self.number_of_point_udc_entry.config(state=NORMAL)
                ...
                self.number_of_point_udc_entry.insert(END, str(self.number_of_point_udc_value))
                self.number_of_point_udc_entry.config(state=DISABLED)
            if self.dc_bias_voltage_level_start_value != self.dc_bias_voltage_level_stop_value:
                try:
                    self.sampling_dc_bias_voltage_level_value = float(self.sampling_dc_bias_voltage_level_entry.get())
                    if self.sampling_dc_bias_voltage_level_value < min_sampling_frequency_value:
                        self.number_of_point_udc_value = 100000
                        self.sampling_dc_bias_voltage_level_value = round(min_sampling_frequency_value, 3)
                        self.sampling_dc_bias_voltage_level_entry.delete(0, END)
                        self.sampling_dc_bias_voltage_level_entry.insert(END,
                                                                         str(self.sampling_dc_bias_voltage_level_value))
                    elif self.sampling_dc_bias_voltage_level_value > max_sampling_frequency_value:
                        self.number_of_point_udc_value = 40
                        self.sampling_dc_bias_voltage_level_value = round(max_sampling_frequency_value, 3)
                        self.sampling_dc_bias_voltage_level_entry.delete(0, END)
                        self.sampling_dc_bias_voltage_level_entry.insert(END,
                                                                         str(self.sampling_dc_bias_voltage_level_value))
                    else:
                        self.number_of_point_udc_value = int(round(
                            (self.dc_bias_voltage_level_stop_value
                             - self.dc_bias_voltage_level_start_value) / self.sampling_dc_bias_voltage_level_value))
                        self.sampling_dc_bias_voltage_level_value = round((self.dc_bias_voltage_level_stop_value
                                                                           - self.dc_bias_voltage_level_start_value)
                                                                          / self.number_of_point_udc_value, 3)
                        self.sampling_dc_bias_voltage_level_entry.delete(0, END)
                        self.sampling_dc_bias_voltage_level_entry.insert(END,
                                                                         str(self.sampling_dc_bias_voltage_level_value))
                    self.number_of_point_udc_entry.config(state=NORMAL)
                    ...
                    self.sampling_dc_bias_voltage_level_value = (self.dc_bias_voltage_level_stop_value
                                                                 - self.dc_bias_voltage_level_start_value) \
                                                                / self.number_of_point_udc_value
                    self.sampling_dc_bias_voltage_level_value = round(self.sampling_dc_bias_voltage_level_value, 3)
                    self.sampling_dc_bias_voltage_level_entry.delete(0, END)
                    self.sampling_dc_bias_voltage_level_entry.insert(END,
                                                                     str(self.sampling_dc_bias_voltage_level_value))
            else:
                ...
                self.sampling_dc_bias_voltage_level_entry.config(state=DISABLED)
                self.number_of_point_udc_entry.config(state=NORMAL)
                if self.number_of_point_udc_value > 100000:
                    self.number_of_point_udc_value = 100000
                    self.number_of_point_udc_entry.delete(0, END)
                    self.number_of_point_udc_entry.insert(END, str(self.number_of_point_udc_value))
                elif self.number_of_point_udc_value < 0:
                    self.number_of_point_udc_value = 0
                    self.number_of_point_udc_entry.delete(0, END)
                    self.number_of_point_udc_entry.insert(END, str(self.number_of_point_udc_value))
        else:
            try:
            ...
            if self.dc_bias_voltage_level_start_value != self.dc_bias_voltage_level_stop_value:
                ...
                self.sampling_dc_bias_voltage_level_entry.config(state=NORMAL)
                self.sampling_dc_bias_voltage_level_entry.delete(0, END)
                self.sampling_dc_bias_voltage_level_entry.insert(END, str(self.sampling_dc_bias_voltage_level_value))
                self.sampling_dc_bias_voltage_level_entry.config(state=DISABLED)
            else:
                self.sampling_dc_bias_voltage_level_entry.config(state=NORMAL)
                self.sampling_dc_bias_voltage_level_entry.delete(0, END)
        ...
        try:
            self.frequency_value = round(float(self.frequency_entry.get()))
            if self.frequency_value < 20:
                self.frequency_value = 20
            elif 20 <= self.frequency_value < 1000:
                self.frequency_value = round(self.frequency_value, 3)
            elif self.frequency_value < 10000:
                self.frequency_value = round(self.frequency_value, 2)
            elif 10000 <= self.frequency_value < 100000:
                self.frequency_value = round(self.frequency_value, 1)
            elif 100000 <= self.frequency_value < 20000000:
                self.frequency_value = round(self.frequency_value, 0)
            else:
                self.frequency_value = 20000000
        except ValueError:
            pass
        ...
        try:
            ...
            self.frequency_start(self.channel, self.frequency_value)
            self.frequency_stop(self.channel, self.frequency_value)
            ...
            self.dc_bias_voltage_level(self.channel, self.dc_bias_voltage_level_start_value)
        except (AttributeError, pyvisa.errors.VisaIOError, pyvisa.errors.InvalidSession):
            self.connect_with_E4990A()
    ...
    def run(self):
        ...
        if self.pc_check_value.get():
            ...
            if self.dataformat_check_value[0].get() == 1:
                header = ["Bias(V)"]
                u_dc_bias = list(np.linspace(self.dc_bias_voltage_level_start_value,
                                             self.dc_bias_voltage_level_stop_value,
                                             int(self.number_of_point_udc_value)))
                u_dc_bias = [round(num, 3) for num in u_dc_bias]
                data_list = []
                for a in range(self.num_of_traces_value.get()):
                    self.select_trace(self.channel, a + 1)
                    if self.get_trace_parameter(self.channel, a + 1) in ["IMP", "ADM"]:
                        header.append(f"{self.get_trace_parameter(self.channel, a + 1)}-Real(S)-data")
                        header.append(f"{self.get_trace_parameter(self.channel, a + 1)}-Imag(S)-data")
                    else:
                        index = self.trace_parameter_list.index(self.get_trace_parameter(self.channel, a + 1))
                        header.append(self.trace_parameter_list_y_axis[index])

                for i in u_dc_bias:
                    try:
                        self.dc_bias_voltage_level(self.channel, i)
                    ...
                    temp = [i]
                    for j in range(self.num_of_traces_value.get()):
                        self.select_trace(self.channel, j + 1)
                        time.sleep(0.2)
                        if self.get_trace_parameter(self.channel, j + 1) in ["IMP", "ADM"]:
                            temp.append(
                                mean([x for i, x in enumerate(self.get_trace_value(self.channel)) if i % 2 == 0]))
                            temp.append(
                                mean([x for i, x in enumerate(self.get_trace_value(self.channel)) if i % 2 != 0]))
                        else:
                            temp.append(
